        ...
        <div className={`w-full ${className || ""}`}>
            ...
            <div className="flex justify-between items-center flex-wrap overflow-hidden">
                <div className="flex items-center flex-wrap">
                    <p className="text-turquoise text-lg lg:text-xl font-bold">{format(product.price * 1.13)}
                        <span className="ml-1 text-[14px] text-black/80">IVAI</span>
                    </p>
                    {product.special_price > 0 ? (
                        <p className="line-through text-[#7B7B7B] text-sm md:text-lg lg:text-xl font-bold">${product.special_price}</p>
                    ) : (
                        <></>
                    )}
                </div>
